...
${capTransferCalls
    ? `- If the caller asks for a person or to be put through, first say e.g. "No problem, I'll transfer you to Jason now." THEN call the transfer_to_staff tool with say_to_caller set to that sentence. Never call the tool silently. Pass staff_name as who they asked for — caller_name is the caller. If they ask for a named person, ring that person — do not ask them for details first. If they ask for the technician without a name, pass staff_name=technician. If the webhook returns no_technician_on_file or could_not_see_job, say so and ask for the name; if they still do not know, call again with name_unknown true. Do not take a message until the webhook returns accepted:false. Only use save_message if the tool returns accepted:false or the transfer fails.`

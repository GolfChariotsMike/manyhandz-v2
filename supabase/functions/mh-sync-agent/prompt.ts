...
export function composeSystemPrompt(data: PromptInput): string {
  ...
  const bookingRule = capConfirmBookings
    ? (calendarConnected
      ? `- BOOKINGS: You can confirm bookings. Use check_calendar_availability then book_calendar_event. Speak success only if the tool returns ok:true. If it fails or says not_connected, take a message — never pretend a booking was made.${createJobOn ? " If they want work done, you MUST still call create_simpro_job once you have the work description (use what they already said — do not ask again if they already described the fault or work) — do not only book a calendar slot." : ""}`
      : `- BOOKINGS: You can confirm bookings. Use your knowledge base to check availability and confirm with callers.${createJobOn ? " If they want work done, you MUST still call create_simpro_job once you have the work description (use what they already said — do not ask again if they already described the fault or work)." : ""}`)
    : (createJobOn
      ? `- BOOKINGS: You CANNOT confirm, reserve, or make any booking. If a caller wants work done or a technician booked, that is a SimPRO lead — FIRST action this turn is lookup_simpro_customer (do not ask name or address until it returns), then create_simpro_job once you have the work description (use what they already said — do not ask again if they already described the fault or work). Do not only take a verbal message. Do not use send_sms to notify the office.`

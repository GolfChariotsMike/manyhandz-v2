...
export function composeSystemPrompt(data: PromptInput): string {
  ...
  const bookingRule = capConfirmBookings
    ? (calendarConnected
      ? `- BOOKINGS: You can confirm bookings. Use check_calendar_availability then book_calendar_event. Speak success only if the tool returns ok:true. If it fails or says not_connected, take a message — never pretend a booking was made.${createJobOn ? " If they want work done, you MUST still call create_simpro_job once you have the work description (use what they already said — do not ask again if they already described the fault or work) and preferred_time — do not only book a calendar slot." : ""}`
      : `- BOOKINGS: You can confirm bookings. Use your knowledge base to check availability and confirm with callers.${createJobOn ? " If they want work done, you MUST still call create_simpro_job once you have the work description (use what they already said — do not ask again if they already described the fault or work) and preferred_time." : ""}`)
    : (createJobOn
      ? `- BOOKINGS: You CANNOT confirm, reserve, or make any booking. If a caller wants work done or a technician booked, that is a SimPRO lead — FIRST action this turn is lookup_simpro_customer (do not ask name or address until it returns), then create_simpro_job once you have the work description (use what they already said — do not ask again if they already described the fault or work) and a preferred time of day if they have not already said one. Do not only take a verbal message. Do not use send_sms to notify the office.`
  ...
  const transferRule = capTransferCalls
    ? `- TRANSFERS: If the caller asks for a person or to be put through (speak to / put me through / technician), SAME TURN call transfer_to_staff. The spoken sentence is the say_to_caller argument — e.g. if they asked for Jason, say_to_caller is "No problem, I'll transfer you to Jason now." Speaking that acknowledgement without calling the tool is a failure. Do not wait. Do not ask "are you still there?". Do not say "one moment" as a substitute for the webhook. Never call the tool silently (empty message + typing is invalid; the webhook rejects a missing say_to_caller). Pass staff_name as who they asked to speak to (first name, full name, or role such as technician or director) — caller_name is the caller, not the destination. If they ask for a named person, ring that person immediately — do not ask them for their name or details first; caller ID is looked up. If they ask for "the technician" or "my technician" and do not give a name, call transfer_to_staff with staff_name set to technician (caller ID is already on the call). If the webhook returns no_technician_on_file or could_not_see_job, say there is none on their file (or you could not see the job) and ask if they know the technician's name. Wait for a name, then call transfer_to_staff with that staff_name. If they still do not know, call transfer_to_staff with name_unknown true. Do not just talk about transferring. Do not take a message until the webhook returns accepted:false. Only use save_message if transfer_to_staff returns accepted:false or the transfer fails.`
...
${capTransferCalls
    ? `- If the caller asks for a person or to be put through, SAME TURN call transfer_to_staff. The spoken sentence is say_to_caller (e.g. "No problem, I'll transfer you to Jason now."). Speaking without the tool is a failure. Never call the tool silently. Do not wait or ask "are you still there?". Pass staff_name as who they asked for — caller_name is the caller. If they ask for a named person, ring that person — do not ask them for details first. If they ask for the technician without a name, pass staff_name=technician. If the webhook returns no_technician_on_file or could_not_see_job, say so and ask for the name; if they still do not know, call again with name_unknown true. Do not take a message until the webhook returns accepted:false. Only use save_message if the tool returns accepted:false or the transfer fails.`

...
export function createSimproJobChatTool(): AnthropicTool {
  return {
    ...
    description:
      "Create a real SimPRO lead (find-or-create customer and site, then create the lead). Always collect their mobile first — there is no caller ID on chat. If that mobile matches an existing customer, skip name and full site address and collect only a short description (optionally confirm site if they volunteer a different address or have multiple sites). New customers: collect name, mobile, site/address, and description. If they said existing but the tool asks for name and address, ask honestly and retry. Tell them the lead number only if ok:true. Never pretend a lead was created. Never look up other customers' leads.",
    input_schema: {
      type: "object",

...
export function lookupSimproCustomerWebhookTool(functionUrl: string): Record<string, unknown> {
  return {
    ...
    description:
      "BOOKING PATH ONLY — not for quotes, job status, transfer, or FAQs. Look up a SimPRO customer by mobile and/or name or business name. Returns the customer and their sites as streets/suburbs — never creates a customer, site, contact, or lead, and never lists jobs. Phone comes from caller ID — do not ask for it; call this as soon as they want to book. HIT: they are existing — never create a new customer. If one site, confirm the street — do not ask for a site ID. If a few sites, ask which street (e.g. 37 Derictoe or 67 Mars) — never site IDs or a numbered 1–20 list. If many sites, ask for the street or suburb and match. After they pick a street, pass simpro_customer_id and that site's site_id to create_simpro_job internally. MISS: ask if they have used the company before; if yes, retry with their name or business name; if no or still no match, collect details and call create_simpro_job. Do not use save_message as the only close when they want work booked.",

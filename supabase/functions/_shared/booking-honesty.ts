...
export function alreadyCollectedRule(channel: "chat" | "voice"): string {
  if (channel === "chat") {
    return `- ALREADY COLLECTED: Once the visitor has given their name, email, mobile, address/suburb, or job description in this conversation, do not ask for those again. If they already said the fault or work (e.g. a technician to look at a Fujitsu, F-A95 fault), that IS the description — pass it to create_simpro_job. Do not ask for a short description of the service needed. Do not ask them to confirm the service description. Only ask if description is still missing. Use what is already in the thread. Chat has no caller ID — that is not a reason to drop a number they already typed.`;
  }
  return `- ALREADY COLLECTED: Once the caller has given their name, email, site/address, or job description in this call, do not ask for those again. If they already said the fault or work (e.g. a technician to look at a Fujitsu, F-A95 fault), that IS the description — pass it to create_simpro_job. Do not ask for a short description of the service needed. Do not ask them to confirm the service description. Only ask if description is still missing. Use what they already said. Phone comes from caller ID — do not ask for it.`;
}
